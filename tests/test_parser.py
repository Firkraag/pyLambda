...
class TestParser(TestCase):
    ...
    def test_delimited(self):
        ...
        self.assertEqual(parser._delimited('{', '}', ',', parser._parse_varname), ['a', 'b', 'c'])
        parser = Parser(TokenStream(InputStream('{}')))
        self.assertEqual(parser._delimited('{', '}', ',', parser._parse_varname), [])
        parser = Parser(TokenStream(InputStream('{a,}')))
        self.assertEqual(parser._delimited('{', '}', ',', parser._parse_varname), ['a'])
        parser = Parser(TokenStream(InputStream('{a,b,}')))
        self.assertEqual(parser._delimited('{', '}', ',', parser._parse_varname), ['a', 'b'])
    ...
    def test_parse_let(self):
        ...
        self.assertEqual(parser._parse_let(), {"type": "call", "func": {
            "type": "lambda",
            "name": "foo",
            "vars": ["a", "b"],
    ...
    def test_parse_vardef(self):
        ...
        self.assertEqual(parser._parse_vardef(), {"name": "a", "def": {"type": "num", "value": 1.0}})
        parser = Parser(TokenStream(InputStream('a')))
    ...
    def test_parse_toplevel(self):
        ...
        self.assertEqual(parser._parse_toplevel(), {'type': 'prog', 'prog': [{'type': 'num', 'value': 1.0},
                                                                            {'type': 'str', 'value': 'a'},
                                                                            {'type': 'var', 'value': 'foo'}]})
        parser = Parser(TokenStream(InputStream('1;"a";foo;')))
        self.assertEqual(parser._parse_toplevel(), {'type': 'prog', 'prog': [{'type': 'num', 'value': 1.0},
                                                                            ...
                                                                            {'type': 'var', 'value': 'foo'}]})
        parser = Parser(TokenStream(InputStream('')))
        self.assertEqual(parser._parse_toplevel(), {'type': 'prog', 'prog': []})
    ...
    def test_parse_prog(self):
        parser = Parser(TokenStream(InputStream('{}')))
        self.assertEqual(parser._parse_prog(), {"type": "bool", "value": False})
        parser = Parser(TokenStream(InputStream('{1;}')))
    ...
    def test_parse_if(self):
        ...
        self.assertEqual(parser._parse_if(), {'type': 'if', 'cond': {'type': 'num', 'value': 1.0},
    ...
    def test_parse_atom(self):
        ...
        self.assertEqual(parser._parse_atom(), {"type": "bool", "value": False})
    ...
    def test_parse_bool(self):
        ...
        parser = Parser(TokenStream(InputStream('false')))
        self.assertEqual(parser._parse_bool(), {"type": "bool", "value": False})

    def test_parse_expression(self):
        parser = Parser(TokenStream(InputStream('1() + "ab"()(1, "ab")')))
        self.assertEqual(parser._parse_expression(), {"type": "call", "func": {
            'type': 'binary',
            'operator': '+',
    ...
    def test_parse_call(self):
        ...
        self.assertEqual(parser._parse_call(func_name), {"type": "call", "func": func_name,
                                                        "args": [{"type": "var", "value": "b"},
                                                                 {"type": "var", "value": "c"}]})

    def test_maybe_call(self):
        parser = Parser(TokenStream(InputStream('a(b, c)')))
        self.assertTrue(parser._maybe_call(parser._parse_varname), {"type": "call", "func": {"type": "var", "value": "a"},
                                                                  "args": [{"type": "var", "value": "b"},
                                                                           {"type": "var", "value": "c"}]})
        parser = Parser(TokenStream(InputStream('a')))
        self.assertTrue(parser._maybe_call(parser._parse_varname), {"type": "var", "value": "a"})
